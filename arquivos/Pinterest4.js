...
// Rota GET
router.get("/search/pinterest4", async (req, res) => {
  ...
  if (type && !["image", "video", "gif"].includes(type.trim().toLowerCase()))
    return res.status(400).json({ status: false, error: "Type parameter must be 'image', 'video', or 'gif'" });

  try {
    const results = await scraper.scrape(query.trim(), type?.trim().toLowerCase() || null);
    res.json({ status: true, data: results, timestamp: new Date().toISOString() });
  } catch (err) {
    console.error(err);
...
// Rota POST
router.post("/api/s/pinterest", async (req, res) => {
